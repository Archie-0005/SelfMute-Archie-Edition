...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_checkmutes", Command_CheckMutes, "Check who you have self-muted");
...
bool MuteSpecial(int client, char[] Argument)
{
	...
	{
		CPrintToChat(client, "{green}[Self-Mute]{default} %t", "sm_friends_nopublic");
		SpecialMute &= ~MUTE_NOTFRIENDS;
		RetValue = true;
	}
	...
	if(SpecialMute)
	{
		...
		CPrintToChat(client, "{green}[Self-Mute]{default} %t", "sm_mute_group", aBuf);
		RetValue = true;
	}
...
bool UnMuteSpecial(int client, char[] Argument)
{
	...
	if(SpecialMute)
	{
		if(SpecialMute & MUTE_ALL)
		{
			...
			else
			{
				for(int i = 1; i <= MaxClients; i++)
				{
					...
					CPrintToChat(client, "{green}[Self-Mute]{default} %t");
					return true;
				}
			}
		}
		...
		CPrintToChat(client, "{green}[Self-Mute]{default} %t", "sm_unmute_group", aBuf);
		return true;
	}
...
public Action Command_SelfMute(int client, int args)
{
	...
	if(TargetCount == 1)
	{
		...
		{
			CPrintToChat(client, "{green}[Self-Mute]{default} %t", "sm_dont_mute_me");
			return Plugin_Handled;
		}
		...
		if(GetExempt(client, aTargetList[0]))
		{
			...
			CPrintToChat(client, "{green}[Self-Mute]{default} You have exempted from self-mute:{green} %", sTargetName);
	...
	CPrintToChat(client, "{green}[Self-Mute]{default} %t", "sm_mute_player", sTargetName);
...
public Action Command_SelfUnMute(int client, int args)
{
	...
	if(TargetCount == 1)
	{
		if(aTargetList[0] == client)
		{
			CPrintToChat(client, "{green}[Self-Mute]{default} %t", "sm_unmute_no_work");
			return Plugin_Handled;
		}
	...
	CPrintToChat(client, "{green}[Self-Mute]{default} %t", "sm_unmute_player", sTargetName);
...
public Action Command_CheckMutes(int client, int args)
{
	...
	if(strlen(aMuted))
	{
		...
		CPrintToChat(client, "{green}[Self-Mute]{default} %t", "sm_mute_player", aMuted);
	}
	...
	if(g_SpecialMutes[client] != MUTE_NONE)
	{
		...
		CPrintToChat(client, "{green}[Self-Mute]{default} %t", "sm_mute_group", aMuted);
	}
	else if(!strlen(aMuted) && !strlen(aExempted))
		CPrintToChat(client, "{green}[Self-Mute]{default} %t", "sm_no_mute_anyone");

	if(strlen(aExempted))
	{
		aExempted[strlen(aExempted) - 2] = 0;
		CPrintToChat(client, "{green}[Self-Mute]{default} You have exempted from self-mute:{green} %s", aExempted);
	}
...
void DisplayMuteMenu(int client)
{
	...
	menu.AddItem("@t", "Dead players");
	menu.AddItem("@dead", "Terrorists");
	menu.AddItem("@alive", "Alive Players");
	if(g_Plugin_AdvancedTargeting)
		menu.AddItem("@!friends", "Friends");
	else
		menu.AddItem("", "", ITEMDRAW_RAWLINE);
...
public int MenuHandler_MuteMenu(Menu menu, MenuAction action, int param1, int param2)
{
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			{
				CPrintToChat(param1, "Internal error: aItem[0] -> %d | Style -> %d", aItem[0], Style);
				return 0;
			}
			...
			if(!client)
			{
				CPrintToChat(param1, "{green}[Self-Mute]{default} %t", "player_nouse");
				menu.DisplayAt(param1, GetMenuSelectionPosition(), MENU_TIME_FOREVER);
				return 0;
			}
			...
			if(GetIgnored(param1, client))
			{
				...
				CPrintToChat(param1, "{green}[Self-Mute]{default} %t", "sm_unmute_player", client);
			}
			else if(GetExempt(param1, client))
			{
				UnExempt(param1, client);
				CPrintToChat(param1, "{green}[Self-Mute]{default} You have removed exempt from self-mute:{green} %N", client);
			}
			else
			{
				Ignore(param1, client);
				CPrintToChat(param1, "{green}[Self-Mute]{default} %t", "sm_mute_player", client);
			}
			menu.DisplayAt(param1, GetMenuSelectionPosition(), MENU_TIME_FOREVER);
			return 0;
		}
		...
		case MenuAction_DisplayItem:
		{
			...
			{
				if(aItem[0] == '@')
					menu.SetTitle("[Self-Mute] %t", "Groups");
				else if(param2 == 0)
					menu.SetTitle("[Self-Mute] %t", "Talking players");
				else
					menu.SetTitle("[Self-Mute] %t", "All players");
			}
